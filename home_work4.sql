...
младше 12 лет
Нужно выбрать количество id из таблицы  media где user_id = id из profile с возрастом меньше 12 и id из media есть в media_id таблицы likes */
SELECT COUNT(id) as 'Количество лайков' FROM media
    WHERE user_id IN
...
женщины.*/
SELECT
	IF ((SELECT COUNT(user_id) FROM likes 
		WHERE user_id in (SELECT user_id FROM profiles WHERE gender='f'))>
        (SELECT COUNT(user_id) FROM likes 
        WHERE user_id in (SELECT user_id FROM profiles WHERE gender='m')), 'женщины','мужчины') as 'пол';
...
(        
	SELECT 
		COUNT(m.id) AS 'Количество сообщений', m.from_user_id AS 'id пользователя',
		CONCAT(u.firstname, ' ', u.lastname) AS 'Пользователь'
		FROM messages as m
